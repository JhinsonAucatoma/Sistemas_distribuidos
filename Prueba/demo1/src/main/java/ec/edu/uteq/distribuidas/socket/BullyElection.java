...
public class BullyElection {
        ...
        static final AtomicInteger liderId = new AtomicInteger
                            ( -1) ;
        ...
        public void run () {
                ...
                System . out . printf ("[P %d] Iniciado . Activo= %b %n", id
                        , activo ) ;

                while ( activo ) {
                    try {
                        Mensaje msg = buzones [ id ]. poll (500 ,
        ...
        void procesarMensaje ( Mensaje msg ) {
                switch ( msg . tipo () ) {
                    case ELECTION -> {
                        System . out . printf ("[P %d] Recibe ELECTION de P %d %n", id , msg . emisor () ) ;// Responder OK al emisor ( soy mas grande )
                        enviar( TipoMensaje . OK , msg . emisor () ) ;
                        // Iniciar mi propia eleccion si no la tengo activa
                        if (! enEleccion ) iniciarEleccion () ;
                        }
                    case OK -> {
                        System . out . printf ("[P %d] Recibe OK de P %d "
                                        + " -> alguien mas grande existe %n", id ,
                                msg . emisor () ) ;
                        enEleccion = false ; // otro tomara el control
                        }
                    case COORDINATOR -> {
            ...
            void iniciarEleccion () {
                enEleccion = true ;
                System . out . printf ("[P %d] Iniciando ELECTION ... %n",
                        id ) ;
                ...
                if (! hayMasGrande ) {
                    ...
                    CompletableFuture. delayedExecutor (1500 ,
                            TimeUnit . MILLISECONDS )
                    . execute (() -> {
                        if ( enEleccion ) {
                            proclamarLider () ;
        ...
        void simularFallo () {
                System . out . printf ("[P %d] *** SIMULANDO FALLO *** %n"
                        , id ) ;
                activo = false ;
                }
            @SuppressWarnings (" unchecked ")
